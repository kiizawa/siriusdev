...
admin socket = /tmp/\$cluster-\$name.asok
pid file = /tmp/\$cluster-\$type.\$id.pid
...
EOF
    ...
    $CEPH_BIN_DIR/monmaptool --create --clobber \
			     --add $HOSTNAME $MON_ADDR --fsid $FSID $CEPH_CONF_DIR/monmap
    ...
    $CEPH_BIN_DIR/ceph-mon -d --mkfs -i $HOSTNAME \
