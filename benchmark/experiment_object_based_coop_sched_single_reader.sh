...
LOG_DIR=$SHARED_LOG_DIR/${NUM_NODES}_${CAPACITY}_${POLICY}_COOP
...
TRACE_LIST=$SHARED_LIST_DIR/reader_synthetic_list/reader_synthetic_list.all
rm -f $TRACE_LIST

for P in $PATTERNS
do
    R_LIST=$SHARED_LIST_DIR/reader_synthetic_list/reader_synthetic_list.$P
    R_LOG=$LOG_DIR/${P}.log
    echo ${R_LIST}","${R_LOG} >> $TRACE_LIST
done
