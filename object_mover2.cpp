...
Session::Session(librados::Rados* cluster) {
  int ret;
  {
    ret = cluster->ioctx_create("storage_pool", io_ctx_storage_);
    if (ret < 0) {
  ...
  {
    ret = cluster->ioctx_create("archive_pool", io_ctx_archive_);
    if (ret < 0) {
      std::cerr << "Couldn't set up ioctx! error " << ret << std::endl;
      abort();
    } else {
...
ObjectMover::ObjectMover(int thread_pool_size, const std::string &trace_filename) {
  ...
  for (int i = 0; i < thread_pool_size; ++i) {
    ...
    sessions_.insert(std::make_pair(t->get_id(), new Session(cluster_)));
  }
